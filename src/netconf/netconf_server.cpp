...
bool NetconfServer::start(const std::string& host, int port) {
    ...
    try {
        ...
        if (nc_server_ssh_endpt_add_hostkey("splitter", "/etc/ssh/ssh_host_rsa_key", -1) != 0) {
            utils::Logger::warning("Failed to add SSH host key, using default");
        }
        
        server_running_ = true;
        server_thread_ = std::make_unique<std::thread>(&NetconfServer::server_thread, this);
...
bool NetconfServer::load_yang_models(const std::vector<std::string>& model_paths) {
...
void NetconfServer::server_thread() {
    ...
    while (server_running_) {
        ...
        utils::Logger::info("New NetConf client connected: {}", client_session->client_info);
    }
    
    utils::Logger::debug("NetConf server thread stopped");
}

void NetconfServer::handle_client_session(std::shared_ptr<ClientSession> client) {
    utils::Logger::debug("Handling NetConf client session: {}", client->client_info);
    ...
    while (client->active && server_running_) {
        ...
        if (msgtype == NC_MSG_ERROR) {
            utils::Logger::error("Error receiving RPC from client {}", client->client_info);
            break;
        }
        ...
        switch (rpc_type) {
            ...
            case NC_RPC_COPYCONFIG:
                ...
                break;
            default:
                result = nc_server_reply_err(client->session, nc_err(NC_ERR_OP_NOT_SUPPORTED));
                break;
        }
        ...
        if (result != 0) {
            utils::Logger::warning("Failed to handle RPC for client {}", client->client_info);
        }
    }
...
int NetconfServer::handle_get_request(struct nc_session* session, const struct nc_rpc* rpc) {
    try {
        std::string state_data = build_state_data_xml();
        
        struct nc_server_reply* reply = nc_server_reply_data(state_data.c_str(), NC_WD_ALL, NC_PARAMTYPE_CONST);
        if (!reply) {
...
int NetconfServer::handle_get_config_request(struct nc_session* session, const struct nc_rpc* rpc) {
    try {
        std::string config_data = build_running_config_xml();
        
        struct nc_server_reply* reply = nc_server_reply_data(config_data.c_str(), NC_WD_ALL, NC_PARAMTYPE_CONST);
        if (!reply) {
            return nc_server_reply_err(session, nc_err(NC_ERR_OP_FAILED));
        }
...
int NetconfServer::handle_edit_config_request(struct nc_session* session, const struct nc_rpc* rpc) {
    try {
...
int NetconfServer::handle_copy_config_request(struct nc_session* session, const struct nc_rpc* rpc) {
    try {
        struct nc_server_reply* reply = nc_server_reply_ok();
        if (!reply) {
            return nc_server_reply_err(session, nc_err(NC_ERR_OP_FAILED));
...
std::string NetconfServer::build_running_config_xml() {
    ...
    oss << "      <description>32-port L-band signal splitter and combiner system</description>\n";
    oss << "    </system>\n";
    oss << "    <ports>\n";
...
std::string NetconfServer::build_state_data_xml() {
    ...
    if (splitter_manager_) {
        ...
        oss << "      <status>\n";
        oss << "        <active-ports>" << stats.active_ports << "</active-ports>\n";
        oss << "        <hardware-healthy>" << (stats.hardware_healthy ? "true" : "false") << "</hardware-healthy>\n";
        oss << "        <total-state-changes>" << stats.total_state_changes << "</total-state-changes>\n";
        oss << "      </status>\n";
    }
...
std::string NetconfServer::build_port_config_xml(int port_id) {
    ...
    oss << "        <auto-enable>" << (config.auto_enable ? "true" : "false") << "</auto-enable>\n";
    oss << "        <min-frequency>" << config.min_frequency_mhz << "</min-frequency>\n";
    oss << "        <max-frequency>" << config.max_frequency_mhz << "</max-frequency>\n";
    oss << "        <gain>" << config.gain_db << "</gain>\n";
    oss << "        <signal-detection-enabled>" << (config.signal_detection_enabled ? "true" : "false") << "</signal-detection-enabled>\n";
    oss << "      </port>\n";
...
std::string NetconfServer::build_port_state_xml(int port_id) {
    ...
    oss << "        <id>" << port_id << "</id>\n";
    oss << "        <name>" << config.name << "</name>\n";
    oss << "        <enabled>" << (state.enabled ? "true" : "false") << "</enabled>\n";
    oss << "        <signal-detected>" << (state.signal_detected ? "true" : "false") << "</signal-detected>\n";
    oss << "        <healthy>" << (state.healthy ? "true" : "false") << "</healthy>\n";
    oss << "        <frequency>" << state.frequency_mhz << "</frequency>\n";
    oss << "        <signal-level>" << state.signal_level_dbm << "</signal-level>\n";
