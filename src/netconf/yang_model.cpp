...
bool YangModel::load_models(const std::vector<std::string>& model_paths) {
    ...
    for (const auto& path : model_paths) {
        ...
        const struct lys_module* module = lys_parse_path(context_, path.c_str(), LYS_IN_YANG);
        if (!module) {
...
bool YangModel::load_model_from_string(const std::string& yang_content) {
    ...
    const struct lys_module* module = lys_parse_mem(context_, yang_content.c_str(), LYS_IN_YANG);
    if (!module) {
...
bool YangModel::validate_config(const std::string& config_xml) {
    ...
    if (lyd_validate_all(&data_tree, context_, LYD_VALIDATE_PRESENT, nullptr) != LY_SUCCESS) {
...
bool YangModel::validate_data(const std::string& data_xml) {
    ...
    if (lyd_parse_data_mem(context_, data_xml.c_str(), LYD_XML, 
                          LYD_PARSE_STRICT, 
                          LYD_VALIDATE_PRESENT, &data_tree) != LY_SUCCESS) {
...
bool YangModel::load_ietf_models() {
    ...
    for (const auto& model_name : ietf_models) {
        const struct lys_module* module = ly_ctx_load_module(context_, model_name.c_str(), nullptr, nullptr);
        if (!module) {
