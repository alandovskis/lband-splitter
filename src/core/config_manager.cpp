...
bool ConfigManager::load_config() {
    if (!std::filesystem::exists(config_file_path_)) {
        utils::Logger::warning("Config file {} not found, creating default", config_file_path_);
    ...
    config_loaded_ = true;
    utils::Logger::info("Configuration loaded successfully from {}", config_file_path_);
    return true;
}
...
bool ConfigManager::save_config() const {
    ...
    try {
        ...
        config["network"] = {
            {"netconf_host", network_config_.netconf_host},
        ...
        config["logging"] = {
            {"log_file", logging_config_.log_file},
        ...
        std::filesystem::create_directories(std::filesystem::path(config_file_path_).parent_path());
        
        std::ofstream file(config_file_path_);
        if (!file) {
            utils::Logger::error("Failed to open config file for writing: {}", config_file_path_);
            return false;
        }
...
bool ConfigManager::validate_config() const {
    if (hardware_config_.gpio_base_pin < 0 || hardware_config_.gpio_base_pin > 500) {
        utils::Logger::error("Invalid GPIO base pin: {}", hardware_config_.gpio_base_pin);
        return false;
    }
    
    if (network_config_.netconf_port <= 0 || network_config_.netconf_port > 65535) {
        utils::Logger::error("Invalid NetConf port: {}", network_config_.netconf_port);
        return false;
    }
    ...
    if (logging_config_.max_file_size_mb <= 0) {
        utils::Logger::error("Invalid log file size: {}", logging_config_.max_file_size_mb);
        return false;
    }
...
nlohmann::json ConfigManager::get_port_configs() const {
...
bool ConfigManager::load_from_file() {
    try {
        ...
        if (raw_config_.contains("hardware")) {
            ...
            hardware_config_.gpio_base_pin = hw.value("gpio_base_pin", hardware_config_.gpio_base_pin);
            hardware_config_.status_led_base_pin = hw.value("status_led_base_pin", hardware_config_.status_led_base_pin);
            hardware_config_.signal_led_base_pin = hw.value("signal_led_base_pin", hardware_config_.signal_led_base_pin);
            hardware_config_.spi_device = hw.value("spi_device", hardware_config_.spi_device);
            hardware_config_.i2c_device = hw.value("i2c_device", hardware_config_.i2c_device);
            hardware_config_.frequency_detector_address = hw.value("frequency_detector_address", hardware_config_.frequency_detector_address);
        }
        
        if (raw_config_.contains("network")) {
            auto net = raw_config_["network"];
            network_config_.netconf_host = net.value("netconf_host", network_config_.netconf_host);
            network_config_.netconf_port = net.value("netconf_port", network_config_.netconf_port);
            network_config_.rest_host = net.value("rest_host", network_config_.rest_host);
            network_config_.rest_port = net.value("rest_port", network_config_.rest_port);
            network_config_.enable_ssl = net.value("enable_ssl", network_config_.enable_ssl);
            network_config_.ssl_cert_path = net.value("ssl_cert_path", network_config_.ssl_cert_path);
            network_config_.ssl_key_path = net.value("ssl_key_path", network_config_.ssl_key_path);
        }
        
        if (raw_config_.contains("logging")) {
            auto log = raw_config_["logging"];
            logging_config_.log_file = log.value("log_file", logging_config_.log_file);
            logging_config_.log_level = log.value("log_level", logging_config_.log_level);
            logging_config_.max_file_size_mb = log.value("max_file_size_mb", logging_config_.max_file_size_mb);
            logging_config_.max_files = log.value("max_files", logging_config_.max_files);
        }
        
        if (raw_config_.contains("monitoring")) {
            auto mon = raw_config_["monitoring"];
            monitoring_config_.enable_telegraf = mon.value("enable_telegraf", monitoring_config_.enable_telegraf);
            monitoring_config_.telegraf_socket = mon.value("telegraf_socket", monitoring_config_.telegraf_socket);
            monitoring_config_.metrics_interval_seconds = mon.value("metrics_interval_seconds", monitoring_config_.metrics_interval_seconds);
            monitoring_config_.enable_health_endpoint = mon.value("enable_health_endpoint", monitoring_config_.enable_health_endpoint);
        }
