...
bool LedController::configure_led_with_pin(int led_id, int gpio_pin) {
    ...
    if (!gpio_controller_->configure_output_pin(gpio_pin)) {
        last_error_ = "Failed to configure GPIO pin " + std::to_string(gpio_pin) + " for LED " + std::to_string(led_id);
        return false;
    }
...
void LedController::start_blink_thread() {
    ...
    blink_thread_ = std::make_unique<std::thread>(&LedController::blink_thread_function, this);
...
std::string LedController::get_last_error() const {
    return last_error_;
}
...
void LedController::update_blinking_leds() {
    ...
    for (auto& [led_id, led_info] : leds_) {
        ...
        if (led_info.state == LedState::BLINKING_SLOW || led_info.state == LedState::BLINKING_FAST) {
            if (should_toggle_led(led_info, now)) {
                ...
                if (success) {
                    ...
                    utils::Logger::warning("Failed to toggle LED {} during blinking", led_id);
                }
            }
        }
    }
...
bool LedController::should_toggle_led(const LedInfo& led_info, std::chrono::steady_clock::time_point now) const {
...
std::chrono::milliseconds LedController::get_blink_interval(LedState state) const {
    switch (state) {
        case LedState::BLINKING_SLOW:
