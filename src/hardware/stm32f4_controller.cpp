...
STM32F4Controller::STM32F4Controller(int port_id, const std::string &uart_device)
...
bool STM32F4Controller::initialize() {
    ...
    try {
        ...
        utils::Logger::info("STM32F4Controller port {} initialized successfully", port_id_);
        return true;
...
bool STM32F4Controller::read_frequency_and_snr(STM32F4Reading &reading) {
    ...
    if (!send_command_with_response(protocol::STM32_CMD_READ_FREQUENCY, freq_response) ||
    ...
    if (freq_response.length >= 4 && snr_response.length >= 4) {
        ...
        reading.frequency_mhz = MIN_FREQUENCY_MHZ + 
            (static_cast<double>(raw_freq) / 65535.0) * (MAX_FREQUENCY_MHZ - MIN_FREQUENCY_MHZ);
        
        reading.snr_db = MIN_SNR_DB + 
            (static_cast<double>(raw_snr) / 65535.0) * (MAX_SNR_DB - MIN_SNR_DB);
        
        reading.signal_level_dbm = -100.0 + (static_cast<double>(raw_freq) / 65535.0) * 60.0;
...
bool STM32F4Controller::start_continuous_measurement() {
    ...
    measurement_thread_ = std::make_unique<std::thread>(&STM32F4Controller::measurement_thread, this);
    
    utils::Logger::info("Started continuous measurement for STM32F4 port {}", port_id_);
    return true;
}
...
bool STM32F4Controller::stop_continuous_measurement() {
    ...
    utils::Logger::info("Stopped continuous measurement for STM32F4 port {}", port_id_);
    return true;
}
...
bool STM32F4Controller::set_led_state(const STM32F4LedState &state) {
    ...
    uint8_t led_data[6] = {
        static_cast<uint8_t>(state.status_led ? 1u : 0u),
    ...
    ResponsePacket response;
    return send_command_with_response(protocol::STM32_CMD_SET_LED, response, led_data, sizeof(led_data));
}
...
bool STM32F4Controller::set_led_blinking(bool status_led, bool signal_led, uint16_t period_ms) {
    STM32F4LedState state;
...
bool STM32F4Controller::update_display(const STM32F4DisplayData &data) {
    ...
    std::string display_text = data.custom_text.empty() ? 
        freq_text + "MHz " + snr_text + "dB" : data.custom_text;
    ...
    ResponsePacket response;
    return send_command_with_response(protocol::STM32_CMD_UPDATE_DISPLAY, response, 
                                    display_data, 3 + display_text.length());
}
...
bool STM32F4Controller::is_healthy() const {
    ...
    auto time_since_comm = std::chrono::duration_cast<std::chrono::seconds>(
        now - last_communication_).count();
    
    return initialized_ && hardware_healthy_ && 
           uart_interface_ && uart_interface_->is_connected() &&
           time_since_comm < 30;
}
...
std::string STM32F4Controller::get_last_error() const {
    return last_error_;
}

bool STM32F4Controller::send_command(uint8_t cmd, const uint8_t *data, size_t data_len) {
    ...
    return uart_interface_->send_data(reinterpret_cast<const uint8_t*>(&packet), packet_size);
}
...
bool STM32F4Controller::send_command_with_response(uint8_t cmd, ResponsePacket &response,
    ...
    if (response.status != protocol::STM32_RESP_OK) {
        ...
        ss << "Command 0x" << std::hex << static_cast<int>(cmd) 
           << " failed with status 0x" << std::hex << static_cast<int>(response.status);
        last_error_ = ss.str();
        return false;
    }
...
void STM32F4Controller::measurement_thread() {
    utils::Logger::debug("STM32F4 measurement thread started for port {}", port_id_);
    ...
    utils::Logger::debug("STM32F4 measurement thread stopped for port {}", port_id_);
}

uint8_t STM32F4Controller::calculate_checksum(const CommandPacket &packet) const {
    uint8_t checksum = 0;
...
bool STM32F4Controller::verify_response_checksum(const ResponsePacket &response) const {
...
STM32F4Controller::UARTInterface::~UARTInterface() {
    cleanup();
}
...
bool STM32F4Controller::UARTInterface::send_data(const uint8_t *data, size_t length) {
    std::lock_guard<std::mutex> lock(uart_mutex_);
...
bool STM32F4Controller::UARTInterface::receive_data(uint8_t *data, size_t max_length, 
...
bool STM32F4Manager::initialize() {
    ...
    for (int port_id = 0; port_id < STM32F4Controller::NUM_PORTS; ++port_id) {
        ...
        if (!configure_port(static_cast<uint8_t>(port_id), uart_device)) {
            utils::Logger::warning("Failed to configure STM32F4 controller for port {}", port_id);
        }
    }

    health_monitoring_ = true;
    health_monitor_thread_ = std::make_unique<std::thread>(&STM32F4Manager::health_monitor_thread, this);
...
bool STM32F4Manager::configure_port(uint8_t port_id, const std::string &uart_device) {
    ...
    controllers_[port_id] = std::make_unique<STM32F4Controller>(port_id, uart_device);
...
void STM32F4Manager::health_monitor_thread() {
    ...
    while (health_monitoring_) {
        ...
        if (!unhealthy_ports.empty()) {
            utils::Logger::warning("Unhealthy STM32F4 ports detected: {}", unhealthy_ports.size());
        }
