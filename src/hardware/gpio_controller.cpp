...
std::string GpioController::get_last_error() const { return last_error_; }
...
bool GpioController::export_pin(int pin) {
  ...
  if (!std::filesystem::exists(get_gpio_path(pin, ""))) {
    last_error_ =
        "GPIO pin " + std::to_string(pin) + " not available after export";
    return false;
  }
...
bool GpioController::write_to_sysfs(const std::string &path,
...
std::string GpioController::get_gpio_path(int pin,
                                          const std::string &file) const {
  std::string base_path =
      std::string(GPIO_BASE_PATH) + "/gpio" + std::to_string(pin);
  if (file.empty()) {
    return base_path;
  }
