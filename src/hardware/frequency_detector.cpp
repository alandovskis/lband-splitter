...
bool FrequencyDetector::initialize() {
    ...
    try {
        ...
        utils::Logger::error("FrequencyDetector initialization failed: {}", e.what());
        return false;
    }
}
...
double FrequencyDetector::get_signal_level(int port_id) const {
    ...
    std::lock_guard<std::mutex> lock(readings_mutex_);
    return readings_[port_id].valid ? readings_[port_id].signal_level_dbm : -100.0;
}
...
void FrequencyDetector::start_continuous_measurement() {
    ...
    measurement_thread_ = std::make_unique<std::thread>(&FrequencyDetector::measurement_thread, this);
...
bool FrequencyDetector::is_healthy() const {
    return initialized_ && hardware_healthy_ &&
           i2c_interface_ && i2c_interface_->is_connected() &&
           spi_interface_ && spi_interface_->is_connected();
}

std::string FrequencyDetector::get_last_error() const {
    return last_error_;
}
...
void FrequencyDetector::measurement_thread() {
    ...
    while (continuous_measurement_) {
        for (int port_id = 0; port_id < NUM_PORTS && continuous_measurement_; ++port_id) {
...
bool FrequencyDetector::measure_port_frequency(int port_id) {
    try {
        ...
        double frequency_mhz = MIN_FREQUENCY_MHZ + 
        ...
        last_error_ = "Exception measuring frequency for port " + std::to_string(port_id) + ": " + e.what();
        hardware_healthy_ = false;
        return false;
    }
}
...
FrequencyDetector::I2CInterface::~I2CInterface() {
    cleanup();
}
...
bool FrequencyDetector::I2CInterface::initialize() {
    ...
    if (ioctl(fd_, I2C_SLAVE, device_address_) < 0) {
        utils::Logger::error("Failed to set I2C slave address: 0x{:02x}", device_address_);
        close(fd_);
        fd_ = -1;
        return false;
    }
...
bool FrequencyDetector::I2CInterface::read_register(uint8_t reg, uint8_t& value) {
    std::lock_guard<std::mutex> lock(i2c_mutex_);
...
bool FrequencyDetector::I2CInterface::write_register(uint8_t reg, uint8_t value) {
    std::lock_guard<std::mutex> lock(i2c_mutex_);
...
bool FrequencyDetector::I2CInterface::read_block(uint8_t reg, uint8_t* data, size_t length) {
    std::lock_guard<std::mutex> lock(i2c_mutex_);
...
FrequencyDetector::SPIInterface::~SPIInterface() {
    cleanup();
}
...
bool FrequencyDetector::SPIInterface::transfer(const uint8_t* tx_data, uint8_t* rx_data, size_t length) {
    std::lock_guard<std::mutex> lock(spi_mutex_);
...
bool FrequencyDetector::SPIInterface::read_adc_channel(int channel, uint16_t& value) {
    if (channel < 0 || channel > 7) {
        return false;
    }
    
    uint8_t tx_data[3] = {
        static_cast<uint8_t>(0x06 | ((channel & 0x04) >> 2)),
        static_cast<uint8_t>((channel & 0x03) << 6),
        0x00
    };
