...
WebSocketServer::WebSocketServer(core::SplitterManager *splitter_manager,
                                 int port)
...
bool WebSocketServer::start(const std::string &host) {
  ...
  try {
    ...
    ws_listener_ = std::make_unique<
        web::websockets::experimental::listener::websocket_listener>(
        uri_builder.to_uri());

    ws_listener_->set_message_handler(
        [this](
            web::websockets::experimental::listener::websocket_incoming_message
                msg) { on_websocket_accept(std::move(msg)); });

    ws_listener_->set_close_handler(
        [this](web::websockets::experimental::listener::websocket_close_status
                   close_status,
...

  {
    ...
    for (auto &client : clients_) {
      try {
        ...
        utils::Logger::warning("Exception closing WebSocket client: {}",
                               e.what());
      }
    }
    clients_.clear();
...
void WebSocketServer::on_websocket_accept(
    web::websockets::experimental::listener::websocket_incoming_message msg) {
  try {
    ...
    utils::Logger::debug("New WebSocket client connected, total clients: {}",
                         clients_.size());
...
void WebSocketServer::on_websocket_close(
    web::websockets::experimental::listener::websocket_close_status
        close_status,
    const utility::string_t &reason) {
  utils::Logger::debug("WebSocket client disconnected: {}",
...
void WebSocketServer::handle_client_message(
    ...
    std::shared_ptr<
        web::websockets::experimental::listener::websocket_callback_client>
        client) {
  try {
    ...
    utils::Logger::warning("Exception handling WebSocket message: {}",
                           e.what());
  }
}

void WebSocketServer::broadcast_port_state_change(int port_id) {
  if (!server_running_ || port_id < 0 ||
      port_id >= core::SplitterManager::NUM_PORTS) {
    return;
  }
  ...
  try {
    ...
    utils::Logger::warning("Exception broadcasting port state change: {}",
                           e.what());
  }
}

void WebSocketServer::broadcast_frequency_change(int port_id,
  ...
  try {
    ...
    utils::Logger::warning("Exception broadcasting frequency change: {}",
                           e.what());
  }
}
  ...
  while (it != clients_.end()) {
    try {
      ...
      utils::Logger::warning(
          "Exception sending WebSocket message, removing client: {}", e.what());
      it = clients_.erase(it);
    }
  }
}

void WebSocketServer::send_message_to_client(
    std::shared_ptr<
        web::websockets::experimental::listener::websocket_callback_client>
        client,
  ...
  try {
    ...
    utils::Logger::warning("Exception sending WebSocket message to client: {}",
                           e.what());
...
web::json::value
WebSocketServer::create_frequency_change_message(int port_id,
                                                 double frequency_mhz) {
  web::json::value message;
...
web::json::value WebSocketServer::create_welcome_message() {
  ...
  message["timestamp"] = web::json::value::string("2024-01-01T00:00:00Z");
  message["server"] =
      web::json::value::string("L-band Splitter WebSocket Server");
