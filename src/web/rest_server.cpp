  ...
  try {
    ...
    http_listener_->support(web::http::methods::GET,
                            [this](web::http::http_request request) {
                              handle_get_request(std::move(request));
                            });
    http_listener_->support(web::http::methods::POST,
                            [this](web::http::http_request request) {
                              handle_post_request(std::move(request));
                            });
    http_listener_->support(web::http::methods::PUT,
                            [this](web::http::http_request request) {
                              handle_put_request(std::move(request));
                            });
    http_listener_->support(web::http::methods::DEL,
                            [this](web::http::http_request request) {
                              handle_delete_request(std::move(request));
                            });
    http_listener_->support(web::http::methods::OPTIONS,
                            [this](web::http::http_request request) {
                              handle_options_request(std::move(request));
                            });

    http_listener_->open().wait();

    websocket_server_ =
        std::make_unique<WebSocketServer>(splitter_manager_, port_ + 1);
...
void RestServer::handle_get_request(web::http::http_request request) {
  ...
  if (path == "/api/v1/system/status") {
    ...
    if (parse_port_id_from_path(path, port_id)) {
      ...
      send_error_response(std::move(request), web::http::status_codes::NotFound,
                          "Endpoint not found");
    }
  }
}
...
void RestServer::handle_delete_request(web::http::http_request request) {
  send_error_response(std::move(request),
                      web::http::status_codes::MethodNotAllowed,
  ...
  try {
    if (!splitter_manager_) {
      send_error_response(std::move(request),
                          ...
                          "Splitter manager not available");
      return;
    }
    ...
    status["total_ports"] =
        web::json::value::number(core::SplitterManager::NUM_PORTS);
    status["hardware_healthy"] =
        web::json::value::boolean(stats.hardware_healthy);
...
void RestServer::get_port_state(web::http::http_request request, int port_id) {
  try {
    ...
    state_json["signal_detected"] =
        web::json::value::boolean(state.signal_detected);
    ...
    state_json["signal_level_dbm"] =
        web::json::value::number(state.signal_level_dbm);
...
void RestServer::get_port_config(web::http::http_request request, int port_id) {
  try {
    ...
    config_json["min_frequency_mhz"] =
        web::json::value::number(config.min_frequency_mhz);
    config_json["max_frequency_mhz"] =
        web::json::value::number(config.max_frequency_mhz);
    config_json["gain_db"] = web::json::value::number(config.gain_db);
    config_json["signal_detection_enabled"] =
        web::json::value::boolean(config.signal_detection_enabled);
...
void RestServer::enable_port(web::http::http_request request, int port_id) {
  try {
    ...
    response["message"] = web::json::value::string(
        success ? "Port enabled successfully" : "Failed to enable port");

    auto status = success ? web::http::status_codes::OK
                          : web::http::status_codes::InternalError;
...
void RestServer::disable_port(web::http::http_request request, int port_id) {
  try {
    ...
    response["port_id"] = web::json::value::number(port_id);
    response["message"] = web::json::value::string(
        success ? "Port disabled successfully" : "Failed to disable port");
...
void RestServer::set_port_config(web::http::http_request request, int port_id) {
  request.extract_json().then([=](web::json::value body) {
    try {
      ...
      response["port_id"] = web::json::value::number(port_id);
      response["message"] = web::json::value::string(
          success ? "Port configuration updated successfully"
                  : "Failed to update port configuration");
...
void RestServer::get_health_check(web::http::http_request request) {
  try {
    bool healthy =
        splitter_manager_ ? splitter_manager_->get_system_health() : false;

    web::json::value health;
    health["status"] =
        web::json::value::string(healthy ? "healthy" : "unhealthy");
    health["timestamp"] = web::json::value::string("2024-01-01T00:00:00Z");

    auto status = healthy ? web::http::status_codes::OK
...
web::json::value
RestServer::port_state_to_json(const core::PortState &state,
                               const core::PortConfig &config) {
  web::json::value port;
...
web::json::value
RestServer::system_stats_to_json(const core::SystemStats &stats) {
  web::json::value metrics;

  metrics["active_ports"] = web::json::value::number(stats.active_ports);
  metrics["total_state_changes"] =
      web::json::value::number(stats.total_state_changes);
  metrics["total_frequency_updates"] =
      web::json::value::number(stats.total_frequency_updates);
  metrics["average_frequency_mhz"] =
      web::json::value::number(stats.average_frequency_mhz);
  metrics["hardware_healthy"] =
      web::json::value::boolean(stats.hardware_healthy);
...
core::PortConfig RestServer::json_to_port_config(const web::json::value &json) {
  ...
  if (json.has_field("min_frequency_mhz") &&
      json["min_frequency_mhz"].is_number()) {
    config.min_frequency_mhz = json["min_frequency_mhz"].as_double();
  }

  if (json.has_field("max_frequency_mhz") &&
      json["max_frequency_mhz"].is_number()) {
  ...
  if (json.has_field("signal_detection_enabled") &&
      json["signal_detection_enabled"].is_boolean()) {
    config.signal_detection_enabled =
        json["signal_detection_enabled"].as_bool();
  }

  return config;
}

bool RestServer::parse_port_id_from_path(const std::string &path,
                                         int &port_id) {
