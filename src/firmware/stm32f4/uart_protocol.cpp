...
void UartProtocol::send_response(Response status, const uint8_t* data, uint8_t length) {
...
void UartProtocol::send_packet(const ResponsePacket& packet) {
    ...
    if (packet.length > 0) {
        std::copy(packet.data.begin(), 
                packet.data.begin() + packet.length,
...
void UartProtocol::uart_rx_complete_callback() {
    uart_rx_ready_ = true;
}
...
bool UartProtocol::verify_checksum(const CommandPacket& packet) {
    uint8_t calculated = static_cast<uint8_t>(packet.command) ^ 
                        packet.port_id ^ 
                        packet.length;
...
extern "C" void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    if (huart->Instance == USART2) {
        // In a real implementation, this would access a global or singleton instance
