...
bool FrequencyDetector::start_measurement() {
  ...
  // Start DMA transfer
  if (HAL_ADC_Start_DMA(&hadc1_, reinterpret_cast<uint32_t *>(adc_buffer_),
...
void FrequencyDetector::process_adc_data() {
  ...
  for (size_t i = 0; i < ADC_BUFFER_SIZE; ++i) {
    sum += adc_buffer_[i];
    if (adc_buffer_[i] < min_val)
      min_val = adc_buffer_[i];
    if (adc_buffer_[i] > max_val)
      max_val = adc_buffer_[i];
  }
  ...
  last_reading_.signal_level_dbm =
      calculate_signal_level_from_raw(raw_frequency);
  last_reading_.port_id = 0; // Single port for now
  last_reading_.valid =
      (peak_to_peak > 100); // Simple signal detection threshold
}

double
FrequencyDetector::calculate_frequency_from_raw(uint16_t raw_value) const {
...
double
FrequencyDetector::calculate_signal_level_from_raw(uint16_t raw_value) const {
...
extern "C" void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
  if (hadc->Instance == ADC1) {
    ...
    // In a real implementation, this would be handled through a singleton or
    // global instance
  }
}