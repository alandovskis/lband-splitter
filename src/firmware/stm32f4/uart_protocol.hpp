...
public:
    ...
    void send_response(Response status, const uint8_t* data = nullptr, uint8_t length = 0);
    
    // Command handler registration
    void set_frequency_handler(FrequencyHandler handler) { frequency_handler_ = std::move(handler); }
    void set_led_handler(LedHandler handler) { led_handler_ = std::move(handler); }
    void set_status_handler(StatusHandler handler) { status_handler_ = std::move(handler); }
    void set_calibration_handler(CalibrationHandler handler) { calibration_handler_ = std::move(handler); }
