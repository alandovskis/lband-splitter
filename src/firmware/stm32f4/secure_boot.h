...
#define FIRMWARE_BANK_SIZE                                                     \
  ((1024 * 1024 - BOOTLOADER_SIZE) / 2) // ~496KB per bank
...
SecureBootResult
secure_boot_verify_signature(const uint8_t *data, uint32_t data_len,
                             const uint8_t *signature,
                             const SecurePublicKey *public_key);
SecureBootResult secure_boot_calculate_hash(const uint8_t *data,
                                            uint32_t data_len,
                                            uint8_t *hash_output);
uint32_t secure_boot_calculate_crc32(const uint8_t *data, uint32_t length);

// Flash operations
SecureBootResult secure_boot_read_flash(uint32_t address, uint8_t *buffer,
                                        uint32_t length);
SecureBootResult secure_boot_write_flash(uint32_t address, const uint8_t *data,
                                         uint32_t length);
...
SecureBootResult secure_boot_verify_both_banks(FirmwareBankId *primary,
                                               FirmwareBankId *fallback);
