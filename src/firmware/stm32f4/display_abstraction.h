...
typedef struct {
  ...
  bool (*write_line)(struct DisplayAbstraction *display, uint8_t line,
                     const char *str);
...
DisplayAbstraction *
display_create_multiplexed(uint8_t port_id, DisplayType type, uint8_t i2c_addr,
                           uint8_t mux_addr, uint8_t mux_channel,
...
bool display_show_two_lines(DisplayAbstraction *display, const char *line1,
                            const char *line2);
bool display_set_enabled(DisplayAbstraction *display, bool enabled);
bool display_update(DisplayAbstraction *display);

// Utility functions
bool display_select_mux_channel(I2C_HandleTypeDef *hi2c, uint8_t mux_addr,
                                uint8_t channel);
