...
void SystemMonitor::start() {
    ...
    monitoring_thread_ = std::make_unique<std::thread>(&SystemMonitor::monitoring_thread, this);
...
void SystemMonitor::send_metrics_to_telegraf() {
    ...
    if (telegraf_socket_fd_ < 0) {
        ...
        strncpy(addr.sun_path, telegraf_socket_path_.c_str(), sizeof(addr.sun_path) - 1);
        
        if (connect(telegraf_socket_fd_, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
            close(telegraf_socket_fd_);
            telegraf_socket_fd_ = -1;
    ...
    try {
        ...
        std::string system_metric = format_telegraf_metric("system", tags, system_fields, now);
        
        std::unordered_map<std::string, double> app_fields{
            {"active_ports", static_cast<double>(app_metrics_.active_ports)},
            {"state_changes", static_cast<double>(app_metrics_.total_state_changes)},
            {"frequency_updates", static_cast<double>(app_metrics_.total_frequency_updates)},
            {"avg_frequency", app_metrics_.average_frequency_mhz},
            {"hardware_healthy", app_metrics_.hardware_healthy ? 1.0 : 0.0}
        };
        
        std::string app_metric = format_telegraf_metric("splitter", tags, app_fields, now);
...
bool SystemMonitor::read_proc_meminfo(double& memory_usage) {
    try {
        ...
        if (total_kb > 0 && available_kb > 0) {
            memory_usage = 100.0 * (1.0 - static_cast<double>(available_kb) / total_kb);
            return true;
        }
...
std::string SystemMonitor::format_telegraf_metric(const std::string& measurement,
