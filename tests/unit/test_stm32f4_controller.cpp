...
public:
    ...
    MOCK_METHOD(bool, receive_data, (uint8_t* data, size_t max_length, size_t& received_length, uint32_t timeout_ms), ());
...
TEST_F(STM32F4ControllerTest, ContinuousMeasurementWhenNotInitialized) {
    EXPECT_FALSE(controller_->start_continuous_measurement());
    EXPECT_TRUE(controller_->stop_continuous_measurement()); // Should succeed even when not started
}
...
TEST(STM32F4Calculations, FrequencyConversion) {
    ...
    double freq_min = min_freq + (static_cast<double>(raw_min) / 65535.0) * (max_freq - min_freq);
    double freq_max = min_freq + (static_cast<double>(raw_max) / 65535.0) * (max_freq - min_freq);
    double freq_mid = min_freq + (static_cast<double>(raw_mid) / 65535.0) * (max_freq - min_freq);
    
    EXPECT_DOUBLE_EQ(freq_min, min_freq);
    EXPECT_DOUBLE_EQ(freq_max, max_freq);
    EXPECT_NEAR(freq_mid, (min_freq + max_freq) / 2.0, 1.0); // Allow 1 MHz tolerance
}
...
TEST(STM32F4Calculations, SNRConversion) {
    ...
    double snr_min = min_snr + (static_cast<double>(raw_min) / 65535.0) * (max_snr - min_snr);
    double snr_max = min_snr + (static_cast<double>(raw_max) / 65535.0) * (max_snr - min_snr);
